...
`,
  ...
  pawn: `# A precious-metals chess set, piece by piece: the pawn. Every man in the
# set is a turned column on a stepped foot, with an enamel roundel let into
# the foot and a pearl, a gem or both above. The two armies are the same
# shapes in different metals — silver with cobalt enamel and sapphires,
# gold with ruby enamel and rubies — so each example shows the pair.
#
# The fluting is engraved, not modelled: a hatch cut into the shaft in
# surface millimetres, which costs nothing and catches the light where a
# plain turned column would only mirror the dark of the room. On a stem the
# hatch runs up the shaft at angle 0; on a revolve — the rook's tower, the
# crowns — it runs up at 90 degrees, since the two surfaces measure their
# millimetres in different directions.
material silver satin
...
# the gold army: the same shapes, a second set of parts, because a part
# carries one material and the two armies are not the same metal
...
form pawn {
  place silverPawn at (-13, 0, 0)
  place goldPawn at (13, 0, 0)
}`,

  rook: `# The rook: a fluted tower with four merlons on its rim, on the same foot
# as the pawn. The tower is a collar — a tube — capped with a disc, so the
# battlement reads as a wall with a floor inside rather than an open ring.
#
# Note the engraving angle. The shaft is a stem and takes its flutes at
# angle 0; the tower is a revolve and takes the same flutes at 90 degrees.
material silver satin
...
form rook {
  place silverRook at (-13, 0, 0)
  place goldRook at (13, 0, 0)
}`,

  knight: `# The knight, drawn as one curved blade: a ribbon swept along a path that
# rises and then folds forward, which is the whole horse — neck, jaw and
# muzzle — in a single stroke. The blade's width lies in the plane the path
# curves in, so the path is the silhouette.
#
# The head points along +Y, which is how the full set turns it to face the
# enemy: the silver knight is placed as drawn, the gold one turned about.
# Here the pair is turned a quarter each way so the two profiles confront
# each other.
material silver satin
...
form knight {
  place silverKnight at (-13, 0, 0) turn -90deg
  place goldKnight at (13, 0, 0) turn 90deg
}`,

  bishop: `# The bishop: a bud for the mitre, cleft by giving it two lobes, with a
# pearl on the point and a gem set in the front of the mitre where a
# bishop's morse would be.
material silver satin
...
form bishop {
  place silverBishop at (-13, 0, 0)
  place goldBishop at (13, 0, 0)
}`,

  queen: `# The queen: a bell for the coronet, mouth upward, with eight stones set
# round its rim on a tilted ring and a pearl held in the middle. The bell's
# throat is wide and its flare gentle, so it sits on the shaft as a cup
...
form queen {
  place silverQueen at (-13, 0, 0)
  place goldQueen at (13, 0, 0)
}`,

  king: `# The king: the queen's coronet, a little lower, under a cross of two bars.
# A bar lies along its own X, so the upright is pitched a quarter turn to
# stand it up; the crossarm is left as it lies.
material silver satin
...
form king {
  place silverKing at (-13, 0, 0)
  place goldKing at (13, 0, 0)
}`,
...
export const exampleGroups: Array<[string, string[]]> = [
  ...
  ['Chess', ['pawn', 'rook', 'knight', 'bishop', 'queen', 'king', 'chessboard', 'chess']],
